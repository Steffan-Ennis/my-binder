...
- [~] T032 [US1] Integration sweep: confirm the sheet feeds end-to-end from the live `useCardDetailQuery` / `useCardPricesQuery` / `useCardPriceHistoryQuery` against the new routes; remove any temporary fixture-feeding used during mock-first dev (`fixtures.ts` stays test-only). (depends on T031 + Phase B) — *Backend half done: routes live + validated; `fixtures.ts` confirmed test-only (no runtime importer); the mobile hooks call the new endpoints. **Live end-to-end render is blocked** by the in-progress mobile chart-deferral cleanup (see note below) — re-verify once mobile compiles/tests green.*
...
> **⚠ Mobile is mid-refactor (chart deferral) — blocks T033/T034/T036.** Deleting the chart left the mobile harness inconsistent: `react-native-gifted-charts` is uninstalled but `apps/mobile/jest.setup.ts` still `jest.mock(...)`s it, so **every mobile test suite fails to load**. There are also 3 stale typecheck errors (`imageUrl` removed from `CardDetailSheetViewProps` but still passed in `CardDetailSheetView.test.tsx` + `CardDetailSheetContainer.test.tsx`; `onCardPress` missing from the `CatalogueView.test.tsx` props mock). Finishing the deferral cleanly (remove the gifted-charts mock, reconcile the sheet view/tests to the no-chart shape, fix the 3 typecheck errors) is mobile work owned by the in-progress refactor — left for the user's call. **Server + core are fully green** (`turbo typecheck` passes for both; 181/181 server tests pass).

- [ ] T033 [P] Coverage config: add `src/components/card-detail-sheet/**`, `src/hooks/useCard{Detail,Prices,PriceHistory}Query.ts`, and `src/utils/priceSeriesToChartData.ts` to the mobile coverage scope in `apps/mobile/jest.config.ts`; confirm the 80/80/80/80 (lines/functions/branches/statements) thresholds hold for the new code. — *Blocked: mobile harness broken (see note); also the chart util/component scope is changing with the deferral.*
- [ ] T034 Run `turbo test` + `turbo typecheck` across all three workspaces (core builds first) — both exit 0, 100% Jest pass rate, coverage thresholds honoured. No `.skip` / `xit` / `it.todo` / quarantine anywhere in the new tests. — *Server + core: ✅ typecheck + 181 tests green. Mobile: ❌ blocked by the chart-deferral cleanup (see note).*
