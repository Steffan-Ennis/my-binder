...
- [X] T024 [P] [US1] Catalogue navigation: update `apps/mobile/src/components/catalogue/useCatalogue.ts` so a populated-pocket press calls `router.navigate` to the `card-detail` route with the tapped `printingId`; skeleton/empty pockets do **not** navigate (Edge Case "Tap during page load"). Add the assertions to `apps/mobile/src/components/catalogue/useCatalogue.test.ts` (FR-001 + no-open-on-skeleton) before implementing. (depends on T022) — *completed manually; `useCatalogue.ts` navigates to `/catalogue/card-detail`.*
- [X] T025 [P] [US1] Binder navigation: update `apps/mobile/src/components/binder-home/useBinderHome.ts` so a populated Binder pocket press opens the sheet for the tapped printing — identical behaviour to the Catalogue (FR-001). Add the assertion to `apps/mobile/src/components/binder-home/useBinderHome.test.ts` first. (depends on T023) — *completed manually; `useBinderHome.ts` navigates to `/binder/card-detail`.*
...
- [X] T026 [P] [US1] Update `apps/server/src/providers/mtgjson/MtgjsonProvider.test.ts` — `getPrices(uuid)` returns the latest CK + TCGP `retail`/`normal` observation or `null` per source; `getPriceHistory(uuid, days)` returns each source's series over the window; **physical-only — digital observations excluded** (FR-006/SC-003); MTG Goldfish never emitted. Also wrap the file's existing top-level describes into one root describe (v1.27.0 carry-over). — *Done. Per the spec's mock-the-SDK decision, `sdk.prices.today`/`history` are spied (the offline cache has no price parquets). Also fixed the `getByUuid` assertion to include the `oracle` field added for the sheet. 13 tests green.*
- [X] T027 [P] [US1] Update `apps/server/src/services/cardService.test.ts` — `getPrices`/`getPriceHistory` orchestration: 30-day window ending today; pass-through of provider `null`/`[]`; not-found propagation. Consolidate into a single root describe (v1.27.0). — *Done. Wrapped in `describe('cardService')`. 33 tests green.*
- [X] T028 [P] [US1] Update `apps/server/src/routes/cards.test.ts` — `GET /cards/:id/prices` → validated `CardPricesResponse`; `GET /cards/:id/prices/history?days=30` → validated `CardPriceHistoryResponse`; default `days=30` when omitted; both-empty is a valid 200; auth gate; 404 on unknown id. Real-pipeline E2E: real TypeORM `DataSource` via `connectTestDatabase()`, offline-mode MTGJSON SDK as the active provider, factory-seeded entities (`apps/server/testing/*Factory.ts`); additions nest inside the file's existing root describe. — *Done. A stub provider stands in for the price layer in the price describe (offline cache has no price parquets); the real provider is restored afterwards. 39 tests green; the file already had a single root `describe('Cards API')`.*

#### Implementation for Phase C

- [X] T029 [US1] Implement `MtgjsonProvider.getPrices` / `getPriceHistory` in `apps/server/src/providers/mtgjson/MtgjsonProvider.ts` (replace the throwing stubs): define the `PriceSource → SDK key` map (`CARD_KINGDOM → 'cardkingdom'`, `TCG_PLAYER → 'tcgplayer'`); fan out two `sdk.prices.today` / `sdk.prices.history` calls (finish `normal`, priceType `retail`, **physical only**); map SDK rows to `CardPricesResponse` / `CardPriceHistoryResponse` (`amountCents` wire unit); return `null` per slot / `[]` per series when no observation; never emit Goldfish. Replace the "not implemented (pending spec 018 US3)" `@throws` JSDoc with real `@returns`/`@example` (Principle IX). Makes T026 pass. — *Done. Sequential fan-out (DuckDB race); `source === 'paper'` filters digital; `amountCents = round(price × 100)`.*
- [X] T030 [US1] Add `getPrices(id)` / `getPriceHistory(id, days = 30)` to `apps/server/src/services/cardService.ts` (beside `getCardImagesById`) — 30-day window ending today; delegate to `providerRegistry.getActive()`; propagate not-found; rewrite provider failure to the existing `ProviderUnavailableError`. JSDoc + `@example` (Principle IX). Makes T027 pass. (depends on T029) — *Done. Existence probe via `getByUuid` distinguishes unknown-printing (404) from known-but-no-observation (200 with `null` slots).*
- [X] T031 [US1] Add `GET /cards/:id/prices` and `GET /cards/:id/prices/history` to `apps/server/src/routes/cards.ts`, **registered before the generic `/cards/:id`** (same precaution the existing literal-segment routes use); validate params + the `days` query (default `30`); attach the `CARD_PRICES_RESPONSE_SCHEMA` / `CARD_PRICE_HISTORY_RESPONSE_SCHEMA` response schemas; same auth gate as `GET /cards/:id`; delegate to `cardService`; map the not-found case to 404 exactly as the sibling `/cards/:id` route does, and `ProviderUnavailableError` → 503. Makes T028 pass. (depends on T030) — *Done.*
- [X] T032 [US1] Integration sweep: confirm the sheet feeds end-to-end from the live `useCardDetailQuery` / `useCardPricesQuery` / `useCardPriceHistoryQuery` against the new routes; remove any temporary fixture-feeding used during mock-first dev (`fixtures.ts` stays test-only). (depends on T031 + Phase B) — *Done. Routes live + validated; the hook composes the three live query hooks (no fixture-feeding — `fixtures.ts` is test-only); mobile compiles + tests green. The trend **chart** is deferred (placeholder rendered); the price-history **data** still flows through `useCardPriceHistoryQuery`.*

> **Spec-020 changes made outside the numbered price tasks (this session):**
> - **Chart deferred** (user decision — the `PriceTrendChart` was crashing the app and has been deleted). The price-history backend (`getPriceHistory` + `GET /cards/:id/prices/history`) was still implemented (user chose "implement both endpoints"); only the mobile chart *rendering* is deferred.
> - **`getCard` fix** (`cardService.ts`): the prior commit `edd3b41` rewrote `getCard` to combine MTGJSON + binder records but returned **500** for any printing not in the MTGJSON cache (unknown id, or an owned card the SDK doesn't know). Rewritten to 404 when neither record exists and to combine what is present otherwise (unowned catalogue printings now return `numberOwned: 0` with no timestamps). `Card.createdAt`/`updatedAt` made optional in `@my-binder/core` (only `getCard` read them). This fixes 4 pre-existing red tests and unblocks the sheet's identity query.
...
> **✅ Mobile chart-deferral cleanup complete.** Removed the dead `react-native-gifted-charts` mock from `apps/mobile/jest.setup.ts` (the package is uninstalled); reconciled the sheet view to the no-chart shape (loading/error/`empty` "no recent price data" annotation kept; a "coming soon" placeholder replaces the chart on `ready`); fixed the 3 typecheck errors (`imageUrl` dropped from the two sheet tests; `onCardPress` added to the `CatalogueView.test.tsx` mock; `id` added to the sheet-view test defaults); wrapped the sheet-view test in a `QueryClientProvider` + active session (the hero now embeds `<CardContainer />`, mirroring `BinderHomeView.test.tsx`); removed the now-unused `PriceTrendChartProps` type. The price-history **data layer** (`useCardPriceHistoryQuery`, `priceSeriesToChartData`, `chartSeries` derivation) is retained intact for when the chart returns.

- [X] T033 [P] Coverage config: add `src/components/card-detail-sheet/**`, `src/hooks/useCard{Detail,Prices,PriceHistory}Query.ts`, and `src/utils/priceSeriesToChartData.ts` to the mobile coverage scope in `apps/mobile/jest.config.ts`; confirm the 80/80/80/80 (lines/functions/branches/statements) thresholds hold for the new code. — *Scope already covered by the `src/**` glob; the `./src/components/card-detail-sheet/` 85% threshold already exists. Verified the slice holds: card-detail-sheet 96.8% lines / 85.93% branches, the three query hooks 100%, `priceSeriesToChartData` 100% lines / 87.5% branches.*
- [X] T034 Run `turbo test` + `turbo typecheck` across all three workspaces (core builds first) — both exit 0, 100% Jest pass rate, coverage thresholds honoured. No `.skip` / `xit` / `it.todo` / quarantine anywhere in the new tests. — *✅ `turbo typecheck` 4/4; `turbo test` 4/4 — core 34, mobile 48 suites/317, server 17 suites/181.*
- [X] T035 Constitution sweep — `FC` declaration rule; style co-location (`*.theme.ts` via `useStyles`, no inline styles); hook return-value memoisation (v1.16.0); data-fetching hook composition (Principle X); Principle IX (pure `card-detail-sheet/index.ts` barrel; JSDoc + `@example` on `cardService` + `MtgjsonProvider` price methods); no new Zustand store (printing carried as the `card-detail` route param only). — *Holds. Single-root-describe (v1.27.0) applied to `MtgjsonProvider.test.ts` + `cardService.test.ts`. The `react-native-gifted-charts` Dependency Currency Check is **N/A** — the chart is deferred and the package was removed.*
- [ ] T036 Manual acceptance per quickstart.md (SC-001/002/003) — *Requires a device/simulator (cannot run headless here). Note: the **30-day chart** items are deferred (the section shows a "coming soon" placeholder); the price-rows / stepper / empty-annotation / error-retry / a11y / physical-only items are exercised by the unit suites and the live routes.* tap a populated pocket on the Catalogue **and** the Binder → sheet with identity + stepper + three rows (Goldfish disabled) within 1s (SC-002); a printing with no observations → both live rows `—` + "no recent price data" annotation (FR-004); kill the network mid-open → inline error + retry in the price/chart section, identity + stepper still usable (FR-009); tap `+` → count + pocket glyph update after `['cards','detail',id]` refetches (FR-007/FR-011), `−` disabled at 0; swipe down → returns to the exact page + scroll (FR-005); VoiceOver/TalkBack announces each source row by name, not colour (FR-010); zero digital-only printings reach any sheet/observation (SC-003).
