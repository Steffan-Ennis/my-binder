...
import type { CardDetails, CardImages, CardRecord, SearchQuery } from '@my-binder/core';
...
const UUID_UNKNOWN = '00000000-0000-0000-0000-000000000000';
...
// A populated CardDetails the price reads use as their "printing exists" probe.
const boltDetails = (uuid: string): CardDetails => ({
...
describe('cardService', () => {
  ...
  describe('getCards/getCard MTGJSON enrichment', () => {
    beforeEach(async () => {
      await aCard().forUser(userA).withId(UUID_BOLT).withName('Lightning Bolt').persist(dataSource);
    ...
    test('getCard decorates a single row with setCode, setName, and typeLine', async () => {
      registry.register('enrich-getcard', makeProvider({
        getByUuid: async (uuid) => uuid === UUID_BOLT
          ? { uuid, name: 'Lightning Bolt', setCode: 'M11', setName: 'Magic 2011', cardNumber: '149', typeLine: 'Instant', oracle: null, scryfallId: null }
          : null,
      }));
    ...
    test('getCard returns the binder row when provider.getByUuid returns null', async () => {
    ...
    test('getCard returns the binder row when provider.getByUuid throws', async () => {
    ...
    test('getCard returns an unowned catalogue printing (no binder row) with numberOwned 0', async () => {
      registry.register('enrich-unowned', makeProvider({
        getByUuid: async (uuid) => boltDetails(uuid),
      }));
      await registry.setActive('enrich-unowned');

      // userB owns nothing here (beforeEach only persisted rows for userA).
      const card = await getCard(UUID_BOLT, userB.id);
      expect(card.id).toBe(UUID_BOLT);
      expect(card.name).toBe('Lightning Bolt');
      expect(card.numberOwned).toBe(0);
  ...
  describe('provider-backed functions', () => {
    beforeAll(async () => {
    ...
    // ─── Spec 020: prices + 30-day history ───────────────────────────────────

    describe('getPrices / getPriceHistory', () => {
      test('getPrices passes the provider response through (null slots allowed)', async () => {
        registry.register('prices-ok', makeProvider({
          getByUuid: async (uuid) => boltDetails(uuid),
        ...
        await registry.setActive('prices-ok');

        const result = await getPrices(UUID_BOLT);
        expect(result.printingId).toBe(UUID_BOLT);
        expect(result.cardKingdom?.amountCents).toBe(1723);
        expect(result.tcgPlayer).toBeNull();
      });

      test('getPriceHistory defaults to a 30-day window and passes [] through', async () => {
        let receivedDays: number | undefined;
        registry.register('history-default', makeProvider({
          getByUuid: async (uuid) => boltDetails(uuid),
        ...
        await registry.setActive('history-default');

        const result = await getPriceHistory(UUID_BOLT);
      ...
      test('getPriceHistory forwards an explicit window length', async () => {
        let receivedDays: number | undefined;
        registry.register('history-explicit', makeProvider({
          getByUuid: async (uuid) => boltDetails(uuid),
          getPriceHistory: async (uuid, days) => {
            receivedDays = days;
            return { printingId: uuid, days, cardKingdom: [], tcgPlayer: [] };
        ...
        await registry.setActive('history-explicit');

        await getPriceHistory(UUID_BOLT, 7);
        expect(receivedDays).toBe(7);
      });

      test('getPrices throws NotFoundError for an unknown printing', async () => {
        registry.register('prices-unknown', makeProvider({ getByUuid: async () => null }));
        await registry.setActive('prices-unknown');
        await expect(() => getPrices(UUID_UNKNOWN)).rejects.toThrow(NotFoundError);
      });

      test('getPriceHistory throws NotFoundError for an unknown printing', async () => {
        registry.register('history-unknown', makeProvider({ getByUuid: async () => null }));
        await registry.setActive('history-unknown');
        await expect(() => getPriceHistory(UUID_UNKNOWN)).rejects.toThrow(NotFoundError);
      });

      test('getPrices rewrites a provider failure to ProviderUnavailableError', async () => {
        registry.register('prices-broken', makeProvider({
          getByUuid: async (uuid) => boltDetails(uuid),
          getPrices: async () => { throw new Error('parquet read failed'); },
        }));
        await registry.setActive('prices-broken');
        await expect(() => getPrices(UUID_BOLT)).rejects.toThrow(ProviderUnavailableError);
      });
    });
  });
});
